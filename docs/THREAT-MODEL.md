...
| Cross-origin prototype control or data access | Exact origin allowlist, sandboxed iframe, capability handshake | Pure protocol session implements origin binding, nonce/session checks, capability negotiation, and contiguous sequencing; browser `postMessage`, source-window binding, and sandbox adapter pending |
