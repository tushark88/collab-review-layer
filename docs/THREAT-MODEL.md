...
| Anchor attached to wrong UI element | Versioned complete admission, immutable context, explicit unavailable/recovery state, generation-correlated reports, and owner-only append-only replacement | Persistence and bridge contract implemented; DOM reattachment pending |
