...
| Cross-origin prototype control or data access | Exact origin allowlist, sandboxed iframe, capability handshake, and shell-owned protected draft controls | Protocol session implements origin binding, nonce/session checks, capability negotiation, contiguous sequencing, and a content-free draft attachment lifecycle that rejects bodies; browser adapter implements exact source-window and target-origin binding; frame host requires transient activation for draft opens, rejects closed-shadow focus boundaries, owns composer DOM/styles/keyboard/submission at the maximum ordinary stacking level, preserves typed text across peer dismissal/unavailability, reclaims focus while hidden, projects child content coordinates through frame borders and axis-aligned scaling, rejects unsupported or invisible/clipped placement and composer hosts that alter viewport-fixed coordinates, remains interactive in an associated modal, enforces cross-origin sources and reviewed browser policy, tolerates only correlated late updates for bounded retired draft IDs, and fails closed on unknown host state; document-lifetime request identity, reentry-safe transactional lifecycle delivery, and idempotent stale dismissals preserve a recreated overlay's bridge; embedded overlays reject local draft composers; Chromium attack suite passes |
